...
test('parseRemoteUrl handles ssh remote', () => {
  assert.deepEqual(parseRemoteUrl('git@github.com:acme/my-repo.git'), {
    owner: 'acme',
    repo: 'my-repo'
  });
});
...
test('parseRemoteUrl handles https remote', () => {
  assert.deepEqual(parseRemoteUrl('https://github.com/acme/my-repo.git'), {
    owner: 'acme',
    repo: 'my-repo'
  });
});
...
test('help output includes core commands and codex cloud url constant is set', () => {
