...
function runCodex(args = []) {
  ...
  if (result.error) {
    if (result.error.code === 'ENOENT') {
      throw new Error('Codex CLI is not installed. Install with: npm i -g @openai/codex');
    }
    throw result.error;
  }
  if (result.status !== 0) {
...
function printHelp() {
  return [
    ...
    '  login          Run `codex --login` to link ChatGPT account',
    '  terminal       Run Codex CLI in terminal (passes extra args)',
...
function runCli(argv, cwd = process.cwd()) {
  ...
  switch (command) {
    ...
    case 'terminal':
      runCodex(argv.slice(3));
      return 'Codex terminal session finished.';
    case 'open':
      return commandOpen();
    case 'status':
