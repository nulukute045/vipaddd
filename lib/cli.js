...
function gitRoot(startPath) {
  return runMaybe(`git -C "${startPath}" rev-parse --show-toplevel`) || null;
}
...
function openUrl(url) {
  ...
  if (platform === 'win32') {
    ...
    if (result.status !== 0) throw new Error(`Could not open browser using cmd.exe start. Open manually: ${url}`);
    return;
  }

  const command = platform === 'darwin' ? 'open' : 'xdg-open';
  const result = spawnSync(command, [url], { stdio: 'ignore' });
  if (result.status !== 0) throw new Error(`Could not open browser using ${command}. Open manually: ${url}`);
}
...
function commandPr(cwd) {
  const info = gitInfo(cwd);
  if (!info.parsed) throw new Error('Origin is not a GitHub URL. Could not construct PR link.');
...
function commandHandoff(cwd) {
  const info = gitInfo(cwd);
  if (!info.remote) throw new Error('No origin remote configured.');
  const push = spawnSync('git', ['-C', info.root, 'push', '-u', 'origin', info.branch], { stdio: 'inherit' });
  if (push.status !== 0) throw new Error('git push failed. Resolve the issue and retry.');
  return `Pushed ${info.branch} to origin. Ready for Codex Cloud handoff.`;
}


function commandConnect(cwd) {
  const info = gitInfo(cwd);
  if (!info.parsed) {
    throw new Error('Origin must be a GitHub repository for connect flow.');
  }

  const repoUrl = `https://github.com/${info.parsed.owner}/${info.parsed.repo}`;
  const cloudUrl = `${CODEX_CLOUD_URL}`;
  return [
    'Connected repository context for Codex Cloud:',
    `- Repo: ${repoUrl}`,
    `- Branch: ${info.branch}`,
    '',
    'Next steps:',
    `1) Push branch: git push -u origin ${info.branch}`,
    `2) Open Codex Cloud: ${cloudUrl}`,
    `3) In Cloud, choose the same GitHub repo/branch to continue.`
  ].join('\n');
}
...
function printHelp() {
  return [
    ...
    '',
    'Cloud commands:',
    '  open           Open Codex Cloud web app in your browser',
    '  cloud          Alias of open',
    '  terminal       Explain terminal-only limitation for Codex Cloud',
    '  connect        Show GitHub repo/branch handoff steps for Cloud',
    '',
...
function runCli(argv, cwd = process.cwd()) {
  ...
  switch (command) {
    ...
    case 'cloud':
      return commandOpen();
    case 'terminal':
      return 'Codex Cloud does not expose a public terminal API. Use codex-web open (web) or official Codex CLI.';
    case 'connect':
      return commandConnect(cwd);
    case 'status':
...
module.exports = { runCli, parseRemoteUrl, printHelp, CODEX_CLOUD_URL };
